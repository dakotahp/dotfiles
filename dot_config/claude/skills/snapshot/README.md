...
Each run **replaces** the previous Session Context section rather than accumulating entries. The historical record lives in Session Logs (see `/log`).
...
- **Independent** — can run alone or before `/log`
- `/resume` reads the Session Context section this skill writes
- `/log` captures a more detailed historical session record in `Session Logs/`
