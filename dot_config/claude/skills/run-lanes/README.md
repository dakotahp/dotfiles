...
Starts every ticket that is ready to build as its own background `/feature` session, with one command. A ticket does not wait for its blockers to merge. It starts as soon as its blockers' work exists on a branch. Ready means the ticket has not started, and one of these is true:

- Every blocker is Done. The ticket starts on trunk.
- After it drops Done blockers, and blockers that another blocker's branch already contains, one blocker is left, and it has an open PR in the same repo. The ticket starts as a git-spice stack on that blocker's branch.

A ticket waits only when two or more unrelated blockers are left, when the one left has no branch yet, or when it is in another repo and not Done.
...
- **Some conditions need you.** A ticket can start its description with `Start after: <condition>`, for example "Start after: APP-4508 deployed to production". `/run-lanes` never decides that on its own. It lists those tickets under "Needs confirmation" and asks you once per run.
- **The planner session is the hub.** The TRD records the session that wrote it as the planner, and each ticket carries a `Planner:` line. A lane session asks the planner its spec questions before it asks you. The planner answers, or asks you. Each lane session comments its own name on its ticket, so the planner can message it back. When a lane opens its PR, it tells the planner, and the planner asks whether to run `/run-lanes`. New sessions start only when you say so. Keep the planner running, and do not rename it or the lane sessions, because the name is the address.
...
- Each blocker is Done, or it is the only one left and has an open PR in the same repo. A blocker in another repo must be Done.
